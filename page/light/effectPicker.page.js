...
const logger = DeviceRuntimeCore.HmLogger.getLogger(
...
class EffectPicker extends AppPage {
  ...
  drawElements() {
    ...
    if (this.state.item.attributes.effect) {
      ...
      const dataList = this.state.item.attributes.effect_list.map((item) => {
        return { name: item };
      });
      ...
      const effectList = this.createWidget(hmUI.widget.SCROLL_LIST, {
        x: 10,
        y: this.state.y + 20,
      ...
      scrollListItemClick(
        effectList,
        this.state.item.attributes.effect_list.indexOf(
          this.state.item.attributes.effect
        ),
        this
      );
    } else {
